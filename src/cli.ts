...
async function main() {
  ...
  try {
    ...
    while (running) {
      ...
      console.log('  2. Read counter value');
      ...
      switch (choice.trim()) {
        case '1': {
          const incrementStr = await rl.question('  Enter increment amount: ');
          const incrementVal = BigInt(incrementStr.trim());
          console.log('\n  Submitting transaction (this may take 30-60 seconds)...');
          try {
            const tx = await deployed.callTx.increment(incrementVal);
            console.log(`\n  ✅ Counter incremented by ${incrementVal}`);
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              ...
              console.log(`\n  📋 Current counter value: ${ledgerState.value}\n`);
            } else {
